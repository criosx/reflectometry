...
def DecomposeRotMatrix(_f1,_f2,OldAlphaOrigin, OldBetaOrigin, OldGammaOrigin):
    ...
    def RotMatrixComp(RotProduct, alpha, beta, gamma):

        a0=numpy.cos(alpha)
        a1=-1*numpy.sin(alpha)
        a2=numpy.sin(alpha)
        a3=numpy.cos(alpha)
        ...
        g0=numpy.cos(gamma)
        g1=-1*numpy.sin(gamma)
        g2=numpy.sin(gamma)
        g3=numpy.cos(gamma)

        CompMatrix=[[a0*g0+a1*b0*g2,a0*g1+a1*b0*g3,a1*b1],[a2*g0+a3*b0*g2,a2*g1+a3*b0*g3,a3*b1],[b2*g2,b2*g3,b3]]
        #print ' '
        #print 'alpha %e beta %e gamma %e \n RotMatrix'
        #print  alpha, beta, gamma
        #print  CompMatrix

        ret = True
        ret = ret and round(RotProduct[0][0],6)==round(CompMatrix[0][0],6)
        ret = ret and round(RotProduct[0][1],6)==round(CompMatrix[0][1],6)
        ret = ret and round(RotProduct[0][2],6)==round(CompMatrix[0][2],6)
        ret = ret and round(RotProduct[1][0],6)==round(CompMatrix[1][0],6)
        ret = ret and round(RotProduct[1][1],6)==round(CompMatrix[1][1],6)
        ret = ret and round(RotProduct[1][2],6)==round(CompMatrix[1][2],6)
        ret = ret and round(RotProduct[2][0],6)==round(CompMatrix[2][0],6)
        ret = ret and round(RotProduct[2][1],6)==round(CompMatrix[2][1],6)
        ret = ret and round(RotProduct[2][2],6)==round(CompMatrix[2][2],6)


        return ret


    f2=numpy.radians(_f2)
    f1=numpy.radians(_f1)
    f20=numpy.radians(OldGammaOrigin)
    f10=numpy.radians(OldBetaOrigin)
    f00=numpy.radians(OldAlphaOrigin)
    ...
    RotZ0_gamma=numpy.array([[numpy.cos(f20), -1*numpy.sin(f20),0],[numpy.sin(f20), numpy.cos(f20),0],[0,0,1]])
    RotX0_beta=numpy.array([[1,0,0],[0,numpy.cos(f10),-1*numpy.sin(f10)],[0,numpy.sin(f10),numpy.cos(f10)]])
    RotZ0_alpha=numpy.array([[numpy.cos(f00), -1*numpy.sin(f00),0],[numpy.sin(f00), numpy.cos(f00),0],[0,0,1]])

    RotProduct=numpy.dot(RotX0_beta,RotZ0_gamma)
    RotProduct=numpy.dot(RotZ0_alpha,RotProduct)
    RotProduct=numpy.dot(RotZ,RotProduct)
    RotProduct=numpy.dot(RotX,RotProduct)
    ...
    f1=numpy.arccos(RotProduct[2][2])

    f0=numpy.arctan(-1*RotProduct[0][2]/RotProduct[1][2])
    if f0<0:
        f0+=numpy.pi
    f0_2=f0+numpy.pi

    ResultCounter=0
    if RotMatrixComp(RotProduct,f0,f1,f2):
        f0f=f0; f1f=f1; f2f=f2;
        ResultCounter+=1;
    if RotMatrixComp(RotProduct,f0,f1,f2_2):
        f0f=f0; f1f=f1; f2f=f2_2;
        ResultCounter+=1;
    if RotMatrixComp(RotProduct,f0_2,f1,f2):
        f0f=f0_2; f1f=f1; f2f=f2;
        ResultCounter+=1;
    if RotMatrixComp(RotProduct,f0_2,f1,f2_2):
        f0f=f0_2; f1f=f1; f2f=f2_2;
        ResultCounter+=1;
    ...
    f2=numpy.degrees(f2f)
    f1=numpy.degrees(f1f)
    f0=numpy.degrees(f0f)

    return f1,f2

def fnBin2D(filename,list1,list2,start1,stop1,step1,start2,stop2,step2,Polar=False,Sigma=0,Deg=True, AvgVariable=[], Avg=False, fileext='LogL',OldAlphaOrigin=0.0, OldBetaOrigin=0.0, OldGammaOrigin=0.0):
    ...
    if Polar==False:
        for (i,k) in enumerate(range(start1,stop1,step1)):
            arr1.append([])
            arr2.append([])
            for (j,m) in enumerate(range(start2,stop2,step2)):
                arr1[i].append(0.)
                arr2[i].append(0.)
    ...
    for i in range(len(list1)):
        ...
        if Polar==False:
            f1, f2 = list1[i], list2[i]
        else:
            f1,f2=DecomposeRotMatrix(list1[i],list2[i],OldAlphaOrigin, OldBetaOrigin, OldGammaOrigin)         #OldOrigin is is how to get from the new origin to the old. In this order!!!
    ...
    if OldBetaOrigin!=0 or OldGammaOrigin!=0:
        filename=filename+'_OldBetaOrigin'+str(OldBetaOrigin)
        filename=filename+'_OldGammaOrigin'+str(OldGammaOrigin)
...
def blur_image(arr, Sigma):

    import scipy.ndimage.filters

    return scipy.ndimage.filters.gaussian_filter(arr, Sigma, mode='nearest')
...
def fnCreatePenetrationPlot(OldAlphaOrigin=0.0, OldBetaOrigin=0.0, OldGammaOrigin=0.0):
    fnCreateLogLPlot(OldAlphaOrigin=OldAlphaOrigin, OldBetaOrigin=OldBetaOrigin, OldGammaOrigin=OldGammaOrigin, filename='sErr.dat', parameter1='beta', parameter2='gamma', parameter3='penetration', start1=0, stop1=93, step1=3, start2=0, stop2=360, step2=5, Polar=True,Sigma=0,Deg=True,fileext='penetration')


def fnCreateLogLPlot(OldAlphaOrigin=0.0, OldBetaOrigin=0.0, OldGammaOrigin=0.0, filename='sErr.dat', parameter1='beta', parameter2='gamma', parameter3='Chisq', start1=0, stop1=93, step1=3, start2=0, stop2=360, step2=5, Polar=True,Sigma=0,Deg=True, fileext='LogL'):
    ...
    print('Analyze Data...')
    fnBin2D(filename,data[parameter1],data[parameter2],start1,stop1,step1,start2,stop2,step2,Polar,Sigma,Deg,data[parameter3],True, fileext, OldAlphaOrigin=OldAlphaOrigin, OldBetaOrigin=OldBetaOrigin,OldGammaOrigin=OldGammaOrigin)


def fnCreateProbabilityPlot(OldAlphaOrigin=0.0, OldBetaOrigin=0.0, OldGammaOrigin=0.0, filename='sErr.dat', parameter1='beta', parameter2='gamma', start1=0, stop1=93, step1=3, start2=0, stop2=360, step2=5, Polar=True,Sigma=0,Deg=True):
    ...
    print('Analyze Data...')
    fnBin2D(filename,data[parameter1],data[parameter2],start1,stop1,step1,start2,stop2,step2,Polar,Sigma,Deg, fileext='prob',OldAlphaOrigin=OldAlphaOrigin, OldBetaOrigin=OldBetaOrigin,OldGammaOrigin=OldGammaOrigin)
