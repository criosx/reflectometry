...
class CRefl1DInteractor(CDataInteractor):
    def __init__(self):
        CDataInteractor.__init__(self)
        pass
...
class CReflectometry:
        ...
        for i in range(len(self.diStatResults['Molgroups'])):  #cycle over all MC iterations
            ...
            for sMolgroup in mgdict.keys():  #cycle over all molgroups
                ...
                if sum(mgdict[sMolgroup]['areaaxis']):
                    fCOM, fInt = average(mgdict[sMolgroup]['zaxis'], weights=mgdict[sMolgroup]['areaaxis'],
                                         returned=True)  #Calculate Center of Mass and Integral
                    fInt = sum(mgdict[sMolgroup]['areaaxis']) * (
                    mgdict[sMolgroup]['zaxis'][1] - mgdict[sMolgroup]['zaxis'][0])
                else:
                    fCOM = 1E5;
                    fInt = 0  #total integral in volume per surface area (unit: Å)
            ...
            if 'tether' in sMolgroups and 'tetherg' in sMolgroups \
                ...
                if 'vf_bilayer' in self.diStatResults['Parameters']:  #check for different nomenclatures of vf_bilayer
                    svfBilayerKey = 'vf_bilayer'
                elif 'vf_bilayer_neat' in self.diStatResults['Parameters']:
                    svfBilayerKey = 'vf_bilayer_neat'

                fVolSubmembrane = mgdict['normarea']['areaaxis'][0] * (float(mgdict['tether']['headerdata']['l']) +
                                                                       float(mgdict['tetherg']['headerdata']['l'])) * \
                                  self.diStatResults['Parameters'][svfBilayerKey]['Values'][i]
                fVolComponents = float(mgdict['bME']['headerdata']['vol']) * float(mgdict['bME']['headerdata']['nf']) + \
                                 float(mgdict['tether']['headerdata']['vol']) * float(
                                     mgdict['tether']['headerdata']['nf']) + \
                                 float(mgdict['tetherg']['headerdata']['vol']) * float(
                                     mgdict['tetherg']['headerdata']['nf']) + \
                                 float(mgdict['headgroup1']['headerdata']['vol']) * float(
                                     mgdict['headgroup1']['headerdata']['nf'])
                ...
                fTotalTetherLength = float(mgdict['tether']['headerdata']['l']) + float(
                    mgdict['tetherg']['headerdata']['l'])
            ...
            if 'lipid1' in sMolgroups and 'methyl1' in sMolgroups:

                fTotalLipid1Length = float(mgdict['lipid1']['headerdata']['l']) + float(
            ...
            if 'lipid2' in sMolgroups and 'methyl2' in sMolgroups:

                fTotalLipid2Length = float(mgdict['lipid2']['headerdata']['l']) + float(
                ...
                fAreaPerLipid2 = float(mgdict['lipid2']['headerdata']['vol']) / float(
    ...
    def fnCreateBilayerPlotData(self):
        ...
        for i in range(len(diIterations['substrate'].keys())-1):
            ...
            sum = substrate + tether + innerhg_corr + hc + outerhg_corr
            ...
            sum_corr = substrate + tether + innerhg_corr + innerhc_corr + outerhc_corr + outerhg_corr
            ...
            sum = substrate + tether + innerhg + innerhc + outerhc + outerhg
...
#-------------------------------------------------------------------------------
    ...
    def fnImportMCMCBestFit(self, sPath):  #imports best-fit from MCMC
        ...
        li_parameters = self.diParameters.keys()
    ...
    def fnPlotMolgroups(self, sPath):
        ...
        try:
            ...
            liParameters = sorted(liParameters, key=lambda keyitem : self.diParameters[keyitem]['number'])  #sort by number of appereance in setup.c
    ...
    try:
        ...
        while (iDoneIterations < iIterations) and (iExitFlag == 0):
            ...
            while (iDoneIterations < (iIterations - len(lSubProcesses) * iIterationsPerCall)) and (
                ...
                file.writelines(data)
                file.close()
                pid = Popen(['qsub', '-q', sQueue, runfile], open(devnull, "w")).communicate()[0]  #ged pid from queue systm
