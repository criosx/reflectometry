...
{  
    ...
    for (i=0; i<iDimensionAtomNameTable; i++)   //loop over whole atom/resiude list
    {
        ...
        {
            iFoundInAtomNameTable=1;
            if (strcmp(cAtomNameTable[i][2],"98") == 0)								//Hydrogen! Calculating nSL from deuteration ratio
            {
...
{
    int iNumber, i, c, iNumberOfStrucInFile;
    ...
    while(!feof(fp)) {
        ...
        if (strcmp(cString,"END") == 0) {
            ...
            if (strcmp (cString,"ATOM") == 0)
            {
                ...
                fscanf(fp,"%5c", cNumber); //reading information
                cNumber[5]='\0';
                fscanf(fp,"%6c", cAtomName); //reading information
                cAtomName[6]='\0';
                while (cAtomName[0]==' ') {
                    for (i=0; i<7; i++) {
                ...
                fscanf(fp,"%3s", cResidue); //reading information
                fscanf(fp,"%s", cChainString); //reading information
                iNumber=atoi(cNumber);
                i=atoi(cChainString);
                
                if (i==0) {                                    //chain identifier present
                ...
                if (iNumberOfStrucInFile==structure) {
                    ...
                    fnFillnSlProfile(dPosZ, cResidue, cAtomName);                 //filling nSL profile
                }
            ...
            else if (strcmp (cString,"HETATM") == 0)
            {
                ...
                if (iNumberOfStrucInFile==structure) {
                    fnFillnSlProfile(dPosZ, cResidue, cAtomType);  //filling nSL profile
...
int fnDetermineLimits(char* strFilename)  //determining coordinate extrema and checking if array size is sufficient
{
        ...
        if (strcmp(cString,"END") == 0) {
            ...
            if (strcmp (cString,"ATOM") == 0)
            {
                ...
                if (iResidueNumber!=iResidueNumberOld)			//make statistic of residues for theoretical Volume and nSLD
                {
                    ...
                    for (i=0; i<20; i++) {if (strcmp (cResidue,cNameAminoAcid[i]) == 0) {iAminoAcidCounter[i]++;}}
                }
    ...
    for (i=0; i<20; i++) 
    { 
        d=iAminoAcidCounter[i];
        dDouble=iResidueNumberCounter;
